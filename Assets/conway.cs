...
public class conway : MonoBehaviour
{
	...
	void Start()
	{
		...
		for (int i = 0; i < height; i++)
			for (int j = 0; j < width; j++)
				texA.SetPixel(i, j, Random.Range(0.0f, 1.0f) < 0.4f ? Color.black : Color.white);
	...
	void Update()
	{
		...
		inputTex = count % speed == 0 ? texA : texB;
		outputTex = count % speed == 0 ? texB : texA;

		if(count % speed ==0)
			rend.material.SetInt("_cI", (count / 20) % 6);	
